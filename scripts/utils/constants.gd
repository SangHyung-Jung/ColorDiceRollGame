...
# === 주사위 컵 설정 ===
const CUP_POSITION := Vector3(10, 5, 0)  # 컵 배치 위치
const CUP_SPAWN_HEIGHT := 3.0  # 주사위 생성 높이
const CUP_SPAWN_RADIUS := 2.0  # 컵 내부 주사위 생성 반경

# === 주사위 물리 시뮬레이션 ===
const DICE_IMPULSE_RANGE := Vector2(10, 50)  # 주사위 쏟을 때 X축 힘의 범위 (더 강하게)
...
const DICE_SPAWN_VELOCITY := 10.0  # 생성 시 초기 하향 속도
const DICE_SETTLEMENT_TIME := 2.5  # 주사위가 컵 바닥에 정착하는 데 필요한 시간 (초)
